...
class CreateUser(graphene.Mutation):
    ...
    def mutate(root, info, email, password, first_name, last_name, full_name,
        ...
        db_create = crud_users.create_user(user=user)
...
class DeleteUser(graphene.Mutation):
    ...
    def mutate(root, info, user_id):
        ...
        message = "success"
        db_update = crud_users.delete_user(user_id=delete_user.user_id)
        if db_update is None:
            raise GraphQLError("Internal Server Error")
        return CreateUser(message=message)
...
class AuthUser(graphene.Mutation):
    ...
    def mutate(root, info, email, password, ip_address, browser):
        db_user = crud_base.get_active_user(email=email)
        ...
        if db_user is None:
            ...
            if is_password_correct is False:
                ...
                crud_login.login_user(user=auth_user, session_id=uid)
