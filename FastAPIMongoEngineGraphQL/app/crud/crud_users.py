...
class CRUDUsers:
    def create_user(self, user: schemas.UserCreate) -> Any:
        ...
        try:
            ...
            db_user = dbconf_models.User(email=user.email,
    ...
    def delete_user(self, user_id: int) -> Any:
        """ Delete User"""
        try:
            data = dbconf_models.User.objects(id=user_id)
            data.delete()
            return True
        except Exception as e:
    ...
    def verify_user(self, email: str) -> Any:
        """ Verify User"""
        try:
            data = dbconf_models.User.objects(email=email).only('id', 'email').first()
            return data
        except Exception as e:
    ...
    def get_user_id(self, user_id: int) -> Any:
        ...
        try:
            data = dbconf_models.User.objects(id=user_id).exclude('password').first()
            return data
        except Exception as e:
    ...
    def get_all_user(self) -> Any:
        """ Get All Users"""
        try:
            query = dbconf_models.User.objects.exclude('password').order_by(
                '-modified_timestamp')
            return query.all()
        except Exception as e:
