...
def get_password(email: str) -> Any:
    ...
    try:
        ...
        data = dbconf_models.User.objects(email=email).only('password').first()
        return data
...
def get_user(email: str) -> Any:
    ...
    try:
        # this will extract user data by excluding password
        data = dbconf_models.User.objects(email=email).exclude('password').first()
        return data
    except Exception as e:
        fastapi_logger.exception("get_user")
        return None
...
def get_active_user(email: str) -> Any:
    ...
    try:
        ...
        #     'password').first()
        data = dbconf_models.User.objects(Q(email=email) & Q(is_active=True)).fields(
            password=0).first()
        return data
    except Exception as e:
        fastapi_logger.exception("get_user")
